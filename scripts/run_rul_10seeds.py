"""
RUL 10-seed training (RAF-DB source, 4 targets).
...
class RUL(nn.Module):
    ...
    def forward(self, x):
        ...
        u = torch.sigmoid(self.uncertainty_head(f))
        return logits, u
...
def train_rul(seed):
    ...
    for ep in range(EPOCHS):
        ...
        for batch in tl:
            ...
            f_mixed = lam * f + (1 - lam) * f[idx]
            u = torch.sigmoid(model.uncertainty_head(f_mixed))
            logits = model.classifier(f_mixed)

            # RUL add-up loss
            loss = lam * crit(logits, labels) + (1-lam) * crit(logits, labels[idx])
            loss = loss + 0.1 * u.mean()  # uncertainty regularization
...
print("="*60)
print("RUL 10-Seed Training")
print(f"New seeds: {NEW_SEEDS}")
print("="*60)

all_results = {}
res_path = OUT / "rul_results.json"
if res_path.exists():
...
print(f"\nRUL: {len(vals)} seeds, Mean={np.mean(vals):.4f} +/- {np.std(vals):.4f}")
